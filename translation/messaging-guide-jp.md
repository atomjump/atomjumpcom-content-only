...
アカウントをお持ちの場合は、最初に（メールアドレスとパスワードを使用して）ログインする必要があります。パスワードボックスの[その他]をクリックし、パスワードを入力して[ログイン]をクリックし、[設定]ページで[登録]をクリックするか、メッセージページで赤い[取り消し線付き]の耳のアイコンをクリックします。

購読済みのフォーラムから退会するには、[設定]をクリックしてから[退会]をクリックするか（ログイン後）、メッセージページの緑色の[耳]アイコンをクリックします。

## グループを設定するには
独自のグループ名、またはAtomJump.comで「Enter a group name」と表示されている既存のパブリックグループ名を入力します。例： 「sailinglondon」をクリックし、「AtomJump」をクリックします。 フォーラムのウェブリンクを共有します。例： 「https://sailinglondon.AtomJump.com」、電子メール、SMS、インスタントメッセージ、またはその他の手段を介してグループと一緒に。 下にあるページの右上隅にある「共有」アイコンをタップすると、atomjump.comでこのリンクをすばやく見つけることができます。 その後、各個人は、継続的な通知を受け取るためにフォーラムのサブスクライバーになることを選択できます。

## 個室を設置するには
AtomJump․comでは、「Enter a group name」と表示されている独自の部屋名を入力できます。例： 「fredfamily」をクリックし、一番下のエントリ「Create a PRIVATE room」をタップします。 AtomJump․comでは、これはNZ $ 15 /年（約$ US 10 /年）で、<a href="https://atomjump.com/wp/introduction/">このページ</にサインアップできます</a>。 他のフォーラムでは、システム管理者に連絡してプライベートフォーラムのパスワードを追加する必要があります。

注：使用することを決定したフォーラムパスワードは、個人パスワードとは異なり、部屋のすべてのメンバーに快適に電子メールで送信できるものでなければなりません。
