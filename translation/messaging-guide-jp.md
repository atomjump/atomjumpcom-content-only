...
## グループを設定するには
独自のグループ名、または既存のパブリックグループ名を入力します。atomjump.comで「グループ名を入力してください」と表示されます。例： 「sailinglondon」をクリックし、「AtomJump」をクリックします。 フォーラムのウェブリンクを共有します。例：「 https://sailinglondon.atomjump.com 」 、電子メール、SMS、インスタントメッセージ、またはその他の手段を介してグループと一緒に。 下にあるページの右上隅にある「共有」アイコンをタップすると、atomjump.comでこのリンクをすばやく見つけることができます。 その後、各個人は、継続的な通知を受け取るためにフォーラムのサブスクライバーになることを選択できます。

## 個室を設置するには
AtomJump․comでは、「グループ名を入力してください」と表示されている独自の部屋名を入力できます。例： 「fredfamily」をクリックし、一番下のエントリ「個室を作成」をタップします。 AtomJump․comでは、これはNZ $ 15 /年（約$ US 10 /年）で、<a href="https://atomjump.com/wp/introduction/">このページ</にサインアップできます</a>。 他のフォーラムでは、システム管理者に連絡してプライベートフォーラムのパスワードを追加する必要があります。
