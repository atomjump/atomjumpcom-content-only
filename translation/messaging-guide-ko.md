...
참고 : 이러한 비디오 포럼은 비공개 포럼에 있지 않는 한 공개되어 있으며 새 사용자가 가입하기 전에 수락해야하지만 이후에는 다른 사람에게 보여줄 내용에주의해야합니다. 여기서 프라이버시를 원하면 일회성 세션을 시작하거나 비공개 포럼을 구입해야합니다.

대체 방법 : 대부분의 플랫폼에서 비디오 포럼을 클릭 할 수 있어야하지만 일부 플랫폼은 특히 iPad / Internet Explorer의 경우 데스크탑 / 앱을 직접 실행하고 포럼 이름을 입력 할 수 있습니다. AtomJump.com 페이지의 경우 포럼 이름은 'aj- [고유 코드] -ajps_'뒤에 하위 도메인이 붙습니다. 마지막‘/’뒤에있는 동영상 링크의 주소를 보면 찾을 수도 있습니다.
...
참고 : 각기 다른 AtomJump Server 설치에는 자체 개인 사용자 데이터베이스가 있습니다. AtomJump.com에서 계정을 설정 한 경우 해당 계정은 모든 atomjump.com 속성에서 일관되지만 찾을 수있는 모든 버전의 메시징 팝업에서 작동하지 않을 수 있습니다. 이는 다른 회사에서 실행하기 때문입니다. .
...
## 그룹을 설정하려면
포럼의 웹 주소를 그룹과 공유하십시오. 기본 페이지의 오른쪽 상단 모서리에있는 '공유'아이콘을 탭하면 atomjump.com에서이를 빠르게 찾을 수 있습니다. 각 개인은 포럼 구독자가되어 지속적인 알림을받을 수 있습니다.

## 비공개 그룹을 설정하려면
atomjump.org에서 그룹 이름을 입력하고 하단 항목 '비공개 그룹 만들기'를 탭할 수 있습니다. 그러면 자동으로 이메일이 채워집니다. 이메일을 보내기 전에 그룹 비밀번호를 교체하십시오. atomjump.org에서는 연간 $ US 10이며이 페이지에서 가입 할 수 있습니다. 다른 포럼에서는 시스템 관리자에게 문의하여 포럼 암호를 추가해야합니다.

참고 :이 포럼 비밀번호는 개인 비밀번호와 다르며 그룹의 모든 구성원에게 편안하게 이메일을 보낼 수있는 비밀번호 여야합니다.
...
여러 포럼에 가입 할 수 있습니다. 위의 '포럼 구독자가 되려면'을 참조하십시오.
...
참고 : '설정'이라는 단어가있는 전면 회색 상자는 주변 사이트에서이 기능을 구현 한 경우에만 언어를 변경합니다. 언어를 변경하면 중앙 상자의 '가이드'메시지가 변경됩니다. 다른 사용자가 입력 한 텍스트에는 영향을주지 않습니다.
